    ...
    */
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "SessionSelected"{
            ...
            svc.session = session
            svc.devices  = session.devices
            svc.lawNum = session.lawNum
            svc.notes = session.notes
            svc.city = session.city
            svc.building = session.bldg
            svc.department = session.dept
            svc.company = session.comp
        }
    }
